...
xml_remove_string() {
	...
	if [ $(sed -e '/name="'$STRING_NAME'"/!d' $XML_TARGET | wc -l) -gt 0 ]; then
		...
		if [ $(sed -e '/name="'$STRING_NAME'"/!d' $XML_TARGET | grep '</string>' | wc -l) -gt 0 ]; then
			sed -e '/<string name="'$STRING_NAME'"/d' $XML_TARGET >>$XML_TARGET.fixed
			mv $XML_TARGET.fixed $XML_TARGET
			write_auto_fix
			...
			sed -e '/<string name="'$STRING_NAME'"/,/string>/d' $XML_TARGET >>$XML_TARGET.fixed
			mv $XML_TARGET.fixed $XML_TARGET
			write_auto_fix
		fi
	fi
...
xml_remove_string_double() {
	...
	if [ $(sed -e '/name="'$STRING_NAME'"/!d' $XML_TARGET | wc -l) -gt 0 ]; then
		...
		if [ $(sed -e '/name="'$STRING_NAME'"/!d' $XML_TARGET | grep '</string>' | wc -l) -gt 0 ]; then
			sed -e '0,/'$STRING_NAME'/{/<string name="'$STRING_NAME'"/d}' $XML_TARGET >>$XML_TARGET.fixed
			mv $XML_TARGET.fixed $XML_TARGET
			write_auto_fix
		elif [ $(sed -e '/name="'$STRING_NAME'"/!d' $XML_TARGET | grep '/>' | wc -l) -gt 0 ]; then
			sed -e '0,/'$STRING_NAME'/{/<string name="'$STRING_NAME'"/d}' $XML_TARGET >>$XML_TARGET.fixed
...
xml_remove_array() {
	...
	if [ $(sed -e '/name="'$ARRAY_NAME'"/!d' $XML_TARGET | wc -l) -gt 0 ]; then
		...
		sed -e '/name="'$ARRAY_NAME'"/,/'$ARRAY_TYPE'/d' $XML_TARGET >>$XML_TARGET.fixed
		mv $XML_TARGET.fixed $XML_TARGET
		write_auto_fix
	fi
}
